...
fod-site:
	...
	@echo "  API:       http://localhost:$(FOD_SITE_PORT)"
	@echo "  Swagger:   http://localhost:$(FOD_SITE_PORT)/docs"
	uv run uvicorn fastopendata.api:app --reload --port $(FOD_SITE_PORT)
...
SNAKEMAKE_CORES ?= 16
FOD_SNAKEMAKE = cd ${FASTOPENDATA_DIR} && uv run snakemake --verbose --resources census_download=1 --cores $(SNAKEMAKE_CORES) --rerun-incomplete
...
subst:
	cat ${FASTOPENDATA_DIR}/fod_input_configs.yaml | envsubst
