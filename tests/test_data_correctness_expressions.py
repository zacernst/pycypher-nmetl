...
class TestComplexStringExpressions:
    ...
    def test_string_function_composition(self, expression_test_context):
        ...
        assert (
            "  b" in first_three
        )  # From "  Bob  " -> "  bob  " -> "  b" (first 3 chars including spaces)
...
class TestAggregationComplexity:
    ...
    def test_conditional_aggregation(self, expression_test_context):
        ...
        assert abs(result["eng_total_salary"].iloc[0] - 190000.0) < 0.001
...
class TestDataIntegrityThroughTransformations:
    ...
    def test_mathematical_consistency(self, expression_test_context):
        """Test mathematical operations maintain consistency through multi-stage WITH."""
        star = Star(context=expression_test_context)
        ...
        # Fixture ages: 25, 30, 30, 35, 28 (5 persons) → sorted: 25, 28, 30, 30, 35
        ages = sorted(result["age_restored"].tolist())
        assert all(isinstance(v, (int, float)) for v in ages)
        assert ages == sorted(ages)  # monotonically non-decreasing

        salaries = result["salary_restored"].tolist()
        assert all(isinstance(v, (int, float)) for v in salaries)
        # All salary_restored values should be multiples of 1000
        assert all(s % 1000 == 0 for s in salaries)
