...
def full_context(
    ...
    knows_relationship_table,
    works_at_relationship_table,
    person_data,
    company_data,
    knows_data,
    works_at_data,
...
class TestEntityTable:
    ...
    def test_entity_table_creates_hash_mappings(self, person_entity_table):
        ...
        assert (
            len(person_entity_table.column_name_to_hash) == 4
        )  # id, name, age, city
...
class TestRelationshipTable:
    ...
    def test_relationship_table_creates_hash_mappings(
        self, knows_relationship_table
    ):
        """Should create hash mappings for all attributes."""
        assert (
            len(knows_relationship_table.column_name_to_hash) == 3
        )  # source_id, target_id, since
        assert len(knows_relationship_table.hash_to_column_name) == 3

    def test_relationship_table_different_entity_types(
        self, works_at_relationship_table
    ):
...
class TestContext:
    ...
    def test_get_entity_table_not_found(self, basic_context):
        ...
        with pytest.raises(
            ValueError, match="Entity table for type Unknown not found"
        ):
    ...
    def test_get_relationship_table_not_found(self, full_context):
        ...
        with pytest.raises(
            ValueError, match="Relationship table for type UNKNOWN not found"
        ):
...
class TestFilter:
    ...
    def test_filter_equality(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="city", value="NYC", operator="="
        )
    ...
    def test_filter_greater_equal(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="age", value=30, operator=">="
        )
        result = filter_op.to_pandas(basic_context)
    ...
    def test_filter_less_equal(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="age", value=28, operator="<="
        )
        result = filter_op.to_pandas(basic_context)
    ...
    def test_filter_unsupported_operator(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="age", value=30, operator="LIKE"
        )
    ...
    def test_filter_invalid_attribute(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="nonexistent", value=123, operator="="
        )

        with pytest.raises(
            ValueError, match="Attribute nonexistent not found"
        ):
            filter_op.to_pandas(basic_context)
    ...
    def test_chained_filters(self, basic_context):
        ...
        filter2 = Filter(
            input=filter1, attribute="city", value="NYC", operator="="
        )
...
class TestJoin:
    ...
    def test_join_inner(self, basic_context):
        ...
        # Join on id column
        join_op = Join(
            left=scan1, right=scan2, left_on=id_hash, right_on=id_hash
        )
    ...
    def test_join_merges_mappings(self, full_context):
        ...
        join_op = Join(
            left=scan1,
            right=scan2,
            left_on=person_id_hash,
            right_on=company_id_hash,
        )
...
class TestProject:
    ...
    def test_project_with_aliases(self, basic_context):
        ...
        # Project with alias
        project = Project(
            input=scan, columns=[name_hash], aliases={name_hash: "person_name"}
        )
        result = project.to_pandas(basic_context)
...
class TestEdgeCases:
    ...
    def test_scan_missing_table(self, basic_context):
        ...
        with pytest.raises(
            ValueError, match="Entity table for type NonExistent not found"
        ):
    ...
    def test_filter_on_empty_dataframe(self, basic_context):
        ...
        scan = Scan(table_type="Person", is_entity=True, variable="p")
        filter_op = Filter(
            input=scan, attribute="age", value=1000, operator=">"
        )
        result = filter_op.to_pandas(basic_context)
...
class TestIntegration:
    ...
    def test_multiple_filters_chained(self, basic_context):
        ...
        filter2 = Filter(
            input=filter1, attribute="age", value=35, operator="<"
        )
        filter3 = Filter(
            input=filter2, attribute="city", value="NYC", operator="="
        )
