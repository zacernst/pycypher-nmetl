...
class TestParserCLI:
    ...
    def test_main_parse_print(self):
        ...
        with (
            patch("sys.argv", ["parser.py", query]),
    ...
    def test_main_ast_json(self):
        ...
        query = "MATCH (n) RETURN n"
        with (
            patch("sys.argv", ["parser.py", "--ast", "--json", query]),
            patch("sys.stdout", new_callable=StringIO) as mock_stdout,
        ):
            main()
            output = mock_stdout.getvalue()
    ...
    def test_main_validate_success(self):
        ...
        query = "MATCH (n) RETURN n"
        with (
            patch("sys.argv", ["parser.py", "--validate", query]),
            patch("sys.stdout", new_callable=StringIO) as mock_stdout,
    ...
    def test_main_file_input(self, tmp_path):
        ...
        with (
            patch("sys.argv", ["parser.py", "--file", str(query_file)]),
            patch("sys.stdout", new_callable=StringIO) as mock_stdout,
        ):
            main()
            output = mock_stdout.getvalue()
    ...
    def test_main_error_handling(self):
        ...
        with (
            patch("sys.argv", ["parser.py", "BAD SYNTAX"]),
            patch("sys.stderr", new_callable=StringIO) as mock_stderr,
            patch("sys.exit") as mock_exit,
        ):
            main()
