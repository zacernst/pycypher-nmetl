...
def context():
    ...
    companies_df = pd.DataFrame(
        {"name": ["Acme", "Globex"], "id": ["c1", "c2"]}
    )

    # Data for KNOWS relationships
    knows_df = pd.DataFrame(
        {"source": ["p1", "p2"], "target": ["p2", "p3"], "since": [2020, 2021]}
    )
...
class TestASTExecution:
    ...
    def test_entity_table_to_pandas(self, context):
        ...
        assert (
            len(df.columns) == 3
        )  # name, age, id (id is not hashed? let's check init)
    ...
    def test_rename_column(self, context):
        ...
        class MockEntityTable(EntityTable):
            ...
            def to_ibis(
                self, ctx
            ):  # Implement abstract method if needed (EntityTable has it)
                pass
        ...
        rename_op = RenameColumn(
            table=mock_table, old_column_name="a", new_column_name="z"
        )
    ...
    def test_drop_column(self, context):
        ...
        drop_op = DropColumn(table=mock_table, column_name="a")
        ...
        drop_op_lazy = DropColumn(
            table=mock_table, column_name="a", execute=False
        )
    ...
    def test_join_pandas(self, context):
        ...
        table_b = MockTableB(
            entity_type="B",
            attributes=["fk"],
            entity_identifier_attribute="fk",
        )
    ...
    def test_multijoin_pandas(self, context):
        ...
        table_a = MockTableA(
            entity_type="A",
            attributes=["k1"],
            entity_identifier_attribute="k1",
        )
        table_b = MockTableB(
            entity_type="B",
            attributes=["k1"],
