...
class TestUndefinedVariables:
    ...
    def test_simple_undefined_variable(self, parser, validator):
        ...
        assert len(errors) > 0
        assert any("'m'" in error.message and "not defined" in error.message 
                   for error in errors)
...
class TestUnwindVariables:
    ...
    @pytest.mark.skip(reason="Grammar doesn't support UNWIND + MATCH without WITH - known limitation")
...
class TestComplexQueries:
    ...
    @pytest.mark.skip(reason="WITH + WHERE syntax edge case - validator needs enhancement for this pattern")
