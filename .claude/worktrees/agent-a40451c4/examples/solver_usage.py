...
def example_4_pycosat():
    ...
    if solution != "UNSAT":
        ...
        print("\nVerifying solution...")
        valid = all(
            any(lit in solution for lit in clause) for clause in clauses
        )
...
def example_6_solution_interpretation():
    ...
    with Glucose3() as sat:
        ...
        if sat.solve():
            ...
            for var_id in model:
                if var_id > 0:
                    ...
                    if isinstance(constraint, VariableAssignedToNode):
                        node_assignments[constraint.variable] = (
                            constraint.node_id
                        )
                    elif isinstance(
                        constraint, VariableAssignedToRelationship
                    ):
                        relationship_assignments[constraint.variable] = (
            ...
            print("\nQuery Result:")
            print("  Pattern (n:Person)-[r:KNOWS]->(m:Person)")
