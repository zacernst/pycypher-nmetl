...
class QueryOptimizer:
    ...
    def __init__(
        self, optimization_level: OptimizationLevel = OptimizationLevel.BASIC
    ):
    ...
    def _evaluate_constant_expression(self, expr: Tree) -> Token | None:
    ...
    def _evaluate_boolean_expression(self, expr: Tree) -> bool | None:
        ...
        if len(expr.children) >= 2:
            for i, child in enumerate(expr.children):
                if isinstance(child, Token):
                    if (
                        child.value.upper() == "AND"
                        and i > 0
                        and i < len(expr.children) - 1
                    ):
    ...
    def _get_numeric_value(self, node) -> float | None:
        ...
        if isinstance(node, Token):
            try:
                ...
                if node.type in ("FLOAT", "DECIMAL_FLOAT"):
    ...
    def _get_boolean_value(self, node) -> bool | None:
        """Extract boolean value from a node."""
        if isinstance(node, Token):
            if node.type == "TRUE" or node.value.lower() == "true":
                return True
            if node.type == "FALSE" or node.value.lower() == "false":
                return False
    ...
    def _push_predicates_down(self, tree: Tree) -> Tree:
        ...
        clauses = (
            list(tree.find_data("match_clause"))
            + list(tree.find_data("with_clause"))
            + list(tree.find_data("where_clause"))
        )
        ...
        for where_clause in where_clauses:
            ...
            for match_clause in reversed(match_clauses):
                ...
                if where_vars.issubset(match_vars):
                    ...
                    if self._can_push_predicate(
                        tree, match_clause, where_clause
                    ):
    ...
    def _can_push_predicate(
        self, tree: Tree, match_clause: Tree, where_clause: Tree
    ) -> bool:
                ...
                if parent:
                    ...
                    if with_index + 1 < len(parent.children):
                        ...
                        if (
                            ...
                            and next_node.data == "where_clause"
                        ):
    ...
    def _find_parent(self, tree: Tree, target: Tree) -> Tree | None:
