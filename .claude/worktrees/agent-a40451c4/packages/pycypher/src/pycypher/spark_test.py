...
if __name__ == "__main__":
    spark = (
        SparkSession.builder.appName("SignalRepartitioner")
        .config(
    ...
    all_data_df = (
        spark.read.parquet("s3a://deva-signal-prod/signal-archive/")
        .filter(
            (col("year") == 2026)
            & (col("month") == 2)
            & (col("day") == 8)
    ...
    out_df = (
        ...
        .groupBy("signal_uuid")
        .agg(countDistinct("signal_value").alias("unique_values"))
        .filter(col("unique_values") > 1)
        .select(col("signal_uuid"), col("unique_values"))
        .sort("unique_values", ascending=False)
        .limit(MAX_SIGNAL_IDS)
        .join(all_data_df, on=["signal_uuid"], how="left")
        .select(
            F.array("window_start_time_ms", "signal_value").alias("time_value")
        )
        .groupBy("signal_uuid")
    ...
    out_df.write.mode("overwrite").parquet(
        "s3://deva-signal-prod/ztest/combined_signals_test_df.parquet"
    )
