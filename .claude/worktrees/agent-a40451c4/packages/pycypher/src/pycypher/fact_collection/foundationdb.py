...
class FoundationDBFactCollection(FactCollection, KeyValue):
    ...
    def _prefix_read_items(
        self,
        prefix: bytes,
        continue_to_end: bool | None = False,
        only_one_result: bool | None = False,
    ) -> Generator[Any, Any]:
    ...
    def enumerate_in_batches(
        ...
        while keys_found:
            ...
            for k, v in self.db.get_range(begin, end, limit=batch_size):
                ...
                counter += 1
            if keys_found:
                begin = fdb.KeySelector.first_greater_than(last_key)  # pyrefly: ignore
        LOGGER.debug("Enumerated %s keys", counter)

    def first_key_after(self, key: bytes, offset: int | None = 1):
        x = self.db.get_range(
            key,
    ...
    def skip_keys(
        self, offset: int | None = 1000, max_keys: int | None = None
    ):
        current_key: bytes = b""
    ...
    def approx_len(
        self, max_keys: int | None = None, increment: int = 10_000
    ):
        counter: int = 0
