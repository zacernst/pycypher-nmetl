...
def test_star_translates_node_without_properties(
    relational_context: Context,
) -> None:
    star = Star(context=relational_context)
    node = NodePattern(
        variable=Variable(name="n"), labels=["Person"], properties={}
    )
...
def test_star_applies_filter_for_node_properties(
    relational_context: Context,
) -> None:
    star = Star(context=relational_context)
    node = NodePattern(
...
def test_star_applies_filter_for_two_node_properties(
    relational_context: Context,
) -> None:
    star = Star(context=relational_context)
    node = NodePattern(
    ...
    assert (
        cast(
            "AttributeEqualsValue",
            cast(typ="FilterRows", val=relation.relation).condition,
        ).left
        == "city"
    )
    assert cast(
...
def test_star_joins_pattern_path(relational_context: Context) -> None:
    ...
    node_a = NodePattern(
        variable=Variable(name="a"), labels=["Person"], properties={}
    )
    node_b = NodePattern(
        variable=Variable(name="b"), labels=["Person"], properties={}
    )
    relationship = RelationshipPattern(
        variable=Variable(name="r"),
        labels=["KNOWS"],
    ...
    path = PatternPath(
        variable=Variable(name="p"), elements=[node_a, relationship, node_b]
    )
    ...
    assert (
        join_relation.join_type == JoinType.INNER
        or join_relation.how == JoinType.INNER
    )
...
def test_from_node_relationship_tail(relational_context: Context) -> None:
    ...
    relation = star._from_node_relationship_tail(
...
def test_from_node_relationship_tail_to_df(
    relational_context: Context,
) -> None:
    star = Star(context=relational_context)
...
def test_integration_1(relational_context: Context) -> None:
    star = Star(context=relational_context)
    cypher: str = (
        "MATCH (n:Person {age: 30})-[k:KNOWS]->(m:Person) RETURN n, m"
    )
    pattern_obj: Pattern = (
        ASTConverter().from_cypher(cypher).clauses[0].pattern
    )
