...
def make_fact_collection(graph: nx.DiGraph):
    ...
    for node in graph_cypher.nodes:
        graph_cypher.nodes[node]['_labels'].append(graph_cypher.nodes[node]['category'])
    ...
    for node in graph_cypher.nodes:
        for label in graph_cypher.nodes[node]['_labels']:
            fact_list.append(FactNodeHasLabel(node_id=graph_cypher.nodes[node]['_id'], node_label=label))
    ...
    for edge in graph_cypher.edges.data():
        ...
        for label in edge_data['_labels']:
            fact_list.append(FactRelationshipHasLabel(relationship_id=edge_data['_id'], relationship_label=label))
        fact_list.append(FactRelationshipHasSourceNode(relationship_id=edge_data['_id'], source_node_id=source_node_id))
        fact_list.append(FactRelationshipHasTargetNode(relationship_id=edge_data['_id'], target_node_id=target_node_id))
