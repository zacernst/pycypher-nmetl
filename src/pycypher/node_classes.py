...
class Aggregation(TreeMixin, Evaluable):
    ...
    def _evaluate(
        ...
        return self.aggregation._evaluate(
...
class Collection(TreeMixin, Evaluable):  # i.e. a list
    ...
    def _evaluate(
        ...
        return Collection(
            [
                value._evaluate(fact_collection, projection=projection)
                for value in self.values
            ]
        )
...
class Distinct(TreeMixin, Evaluable):
    ...
    def _evaluate(
        ...
        """evaluate the COLLECTION, then remove duplicates"""
...
class Collect(TreeMixin, Evaluable):
    ...
    def _evaluate(
        ...
        for matching_instance in matching_instances:
            ...
            single_value = self.object_attribute_lookup._evaluate(
                fact_collection, projection=individuated_projection
            )
            output.append(single_value)
        result = Collection(output)._evaluate(
            fact_collection, projection=projection
        )
        return result
...
class Alias(TreeMixin, Evaluable):
    ...
    def _evaluate(
        ...
        result = self.reference._evaluate(
            fact_collection, projection=projection
        )
        return result
...
class Return(TreeMixin):
    """Represents the ``RETURN`` clause, which is a set of projections."""
