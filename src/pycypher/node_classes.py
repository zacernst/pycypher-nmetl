...
class Evaluable(abc.ABC):  # pylint: disable=too-few-public-methods
    ...
    @abc.abstractmethod
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        pass

    def evaluate(self, *args, **kwargs):
        """Calls the `_evaluate` method and returns the value of the `Literal` object."""
        return self._evaluate(*args, **kwargs).value
...
class Aggregation(TreeMixin, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        return self.aggregation._evaluate(fact_collection, projection=projection)
...
class Collect(TreeMixin, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        return self.object_attribute_lookup._evaluate(fact_collection, projection=projection)
...
class Equals(BinaryBoolean, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        left_value = self.left_side.evaluate(fact_collection, projection=projection)  # pylint: disable=protected-access
        right_value = self.right_side.evaluate(fact_collection, projection=projection)  # pylint: disable=protected-access
...
class Multiplication(Predicate, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        left_value = self.left_side._evaluate(fact_collection, projection=projection)  # pylint: disable=protected-access
        right_value = self.right_side._evaluate(fact_collection, projection=projection)  # pylint: disable=protected-access
        self.type_check(left_value, right_value)
...
class Alias(TreeMixin, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        ...
        result = {self.alias: self.reference._evaluate(fact_collection, projection=projection)}  # pylint: disable=protected-access
...
class ObjectAsSeries(TreeMixin, Evaluable):
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        return [obj._evaluate(fact_collection, projection=projection) for obj in self.object_attribute_lookup_list]
...
class WithClause(TreeMixin, Evaluable):
    ...
    def bak_unique_non_aggregated_variable_solutions(
        self, fact_collection: FactCollection
    ):
        solutions = self.parent.solutions(fact_collection)
        non_aggregated_variable_solutions = []
        for solution in solutions:
            non_aggregated_variable_solution = {}
            for variable in self.non_aggregated_variables:
    ...
    def _evaluate(self, fact_collection: FactCollection, projection: Optional[Dict[str, str | List[str]]] = None) -> Any:
        return self.object_as_series._evaluate(fact_collection, projection=projection)
