...
class Relation(BaseModel):
    ...
    variable_type_map: VariableTypeMap = Field(default_factory=dict)  # e.g. "node" or "relationship"
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if not self.column_names:
            LOGGER.warning(msg=f"Relation {type(self).__name__} created without column_names specified.")
        ...
        self.variable_type_map: VariableTypeMap = {
            var_name: var_type for var_name, var_type in self.variable_type_map.items()
        }
...
class RelationIntersection(Relation):
    ...
    def variables_in_common(self):
        ...
        if not self.relation_list:
            return set()
        variables_in_common: set[Variable] = set(self.relation_list[0].variable_map.keys())
        for relation in self.relation_list[1:]:
            variables_in_common.intersection_update(relation.variable_map.keys())
...
class EntityTable(Relation):
    ...
    attribute_map: dict[Attribute, ColumnName] = Field(
...
class Projection(Relation):
    ...
    def to_pandas(self, context: Context) -> pd.DataFrame:
        ...
        projected_df: pd.DataFrame = base_df.rename(columns=self.projected_column_names)[list(self.projected_column_names.values())]
        return projected_df
...
class FilterRows(Relation):
    ...
    column_map: dict[ColumnName, ColumnName] = Field(
        default_factory=dict
    )
    ...
    def to_pandas(self, context: Context) -> pd.DataFrame:
        ...
        match self.condition:
            case AttributeEqualsValue():
                ...
                condition_value: Any = self.condition.right.evaluate()
                entity_type: EntityType = self.relation.variable_type_map[next(iter(self.relation.variable_map.keys()))]

                entity_df: pd.DataFrame = context.entity_mapping[entity_type].to_pandas(context=context)  # Ensure the entity table is loaded as a DataFrame
                attribute_column_name: ColumnName = context.entity_mapping[entity_type].attribute_map[condition_attribute]
                filtered_entity_df: pd.DataFrame = entity_df[
                    entity_df[str(object=attribute_column_name)] == condition_value
                ]

                # Get the column name holding the right entity ID in the base relation
                ...
                for var, col in self.relation.variable_map.items():
                    ...
                    raise ValueError(f"No variable of type {entity_type} found in relation's variable_type_map.")

                filtered_df = pd.merge(
                    filtered_entity_df, 
                    base_df, 
                    left_on=[ID_COLUMN], 
                    right_on=[base_entity_id_column], 
                    how='inner'
            ...
            case _:
                raise NotImplementedError(f"Condition type {type(self.condition)} not implemented yet.")
