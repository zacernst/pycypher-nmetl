...
class Scan(Algebraic):
    ...
    def to_pandas(self, context: Context) -> pd.DataFrame:
        ...
        if self.table_type not in context.obj_map:
            if self.is_entity:
                raise ValueError(
                    f"Entity table for type {self.table_type} not found in context"
                )
            else:
                raise ValueError(
                    f"Relationship table for type {self.table_type} not found in context"
                )
        ...
        if self.variable and self.is_entity:
            id_hash = table_schema.column_name_to_hash[
                table_schema.entity_identifier_attribute
            ]
...
class Join(Algebraic):
    ...
    def to_pandas(self, context: Context) -> pd.DataFrame:
        ...
        self.column_name_to_hash = {
            **self.left.column_name_to_hash,
            **self.right.column_name_to_hash,
        }
        self.hash_to_column_name = {
            **self.left.hash_to_column_name,
            **self.right.hash_to_column_name,
        }
        self.variables_to_columns = {
            **self.left.variables_to_columns,
            **self.right.variables_to_columns,
        }
...
class QueryTranslator:
    ...
    def __init__(self, context: Context):
        ...
        self.variable_scopes: Dict[
            str, str
        ] = {}  # variable -> entity_type mapping
    ...
    def translate(self, tree: Tree) -> Algebraic:
        ...
        if not match_clause:
            raise ValueError(
                "No match_clause found (only MATCH queries supported)"
            )
    ...
    def _extract_label_name(self, node: Tree) -> Optional[str]:
        ...
        for label_name_node in node.find_data("label_name"):
            if label_name_node.children and isinstance(
                label_name_node.children[0], Token
            ):
    ...
    def _translate_match(self, match_clause: Tree) -> Algebraic:
        ...
        for elem in pattern_elements:
            ...
            for node_pattern in elem.find_data("node_pattern"):
                node_result, node_var = self._translate_node_pattern(
                    node_pattern
                )
            ...
            for rel_pattern in elem.find_data("relationship_pattern"):
                rel_result, rel_var, target_var = (
                    self._translate_relationship_pattern(
    ...
    def _translate_node_pattern(
        self, node_pattern: Tree
    ) -> Tuple[Algebraic, Optional[str]]:
    ...
    def _translate_relationship_pattern(
        ...
        raise NotImplementedError(
            "Relationship pattern translation not fully implemented"
        )

    def _translate_where(
        self, where_clause: Tree, input_algebra: Algebraic
    ) -> Algebraic:
    ...
    def _translate_return(
        self, return_clause: Tree, input_algebra: Algebraic
    ) -> Algebraic:
        ...
        for return_item in return_clause.find_data("return_item"):
            ...
            if found_vars and found_props:
                ...
                if var_name in self.variable_scopes:
                    ...
                    try:
                        ...
                        if prop_name in table.column_name_to_hash:
                            columns.append(
                                table.column_name_to_hash[prop_name]
                            )
                    except ValueError:
                        pass
                ...
                if var_name in input_algebra.variables_to_columns:
                    columns.append(
                        input_algebra.variables_to_columns[var_name]
                    )
