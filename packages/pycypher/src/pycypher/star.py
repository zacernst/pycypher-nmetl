...
from pycypher.relational_models import (
    ...
    SelectColumns,
    FilterRows,
    AttributeEqualsValue, ColumnName,
)
import pandas as pd
...
class Star:
    ...
    def to_relation(self, obj: Algebraizable | str) -> Relation:
        ...
        match obj:
            case str():
                pattern_obj: Pattern = self.to_relation(ASTConverter().from_cypher(obj))
            ...
            case RelationshipPattern(  # -[r:KNOWS]->
                ...
                LOGGER.debug(msg="Translating RelationshipPattern with no properties.")
                out: RelationshipTable = self._from_relationship_pattern(
                    relationship=obj
                )
            case RelationshipPattern(variable=_, labels=_, properties=properties) if (
                len(properties) >= 1
            ):
                LOGGER.debug(
    ...
    def _from_pattern(self, pattern: Pattern) -> Relation:
        ...
        '''
        elements: list[Relation] = [self.to_relation(obj=element) for element in pattern.paths]
        if not elements:
            ...
            LOGGER.debug(msg=f"Pattern has {len(elements)} elements, performing pairwise joins.")
            ...
            for index, first_element in enumerate(elements[:-1]):
                ...
                pairwise_joined_relations.append(self._binary_join(left=first_element, right=second_element))

        out: Relation = self._from_intersection_list(relations=pairwise_joined_relations)
        return out
        ...
        if not elements:
            ...
            for index, first_element in enumerate(elements[:-1]):
                ...
                match (
                    ...
                    case (NodePattern() as node_pattern, RelationshipPattern() as relationship_pattern):
                        normalized_pair = (node_pattern, relationship_pattern)
                        joiner = self._from_node_relationship_tail if relationship_pattern.direction == RelationshipDirection.RIGHT else self._from_node_relationship_head
                    case (RelationshipPattern() as relationship_pattern, NodePattern() as node_pattern):
                        normalized_pair = (node_pattern, relationship_pattern)
                        joiner = self._from_node_relationship_tail if relationship_pattern.direction == RelationshipDirection.LEFT else self._from_node_relationship_head
                    case _:
                        raise ValueError(
                ...
                pairwise_joined_relations.append(
                    joiner(node=node_pattern, relationship=relationship_pattern)
                )
            relation_intersection: RelationIntersection = RelationIntersection(relation_list=pairwise_joined_relations)
            ...
            for relation in relation_intersection.relation_list[1:]:
                join_variables = RelationIntersection(relation_list=[joined_relation, relation]).variables_in_common()
                left_join_on_map = {join_variable: joined_relation.variable_map[join_variable] for join_variable in join_variables}
                right_join_on_map = {join_variable: relation.variable_map[join_variable] for join_variable in join_variables}
                joined_relation: Join = Join(
                    ...
                    variable_map={**joined_relation.variable_map, **relation.variable_map},
                    variable_type_map={**joined_relation.variable_type_map, **relation.variable_type_map},
                    column_names=joined_relation.column_names + relation.column_names,
            ...
            projected_joined_relation = Projection(
                ...
                projected_column_names={
                    column_name: column_name for column_name in joined_relation.variable_map.values()
                },
                source_algebraizable=joined_relation.source_algebraizable,
                variable_map=copy.deepcopy(joined_relation.variable_map),
                variable_type_map=copy.deepcopy(joined_relation.variable_type_map),
                column_names=[column_name for column_name in joined_relation.variable_map.values()],
            )
            import pdb; pdb.set_trace()



    def _from_node_relationship_tail(self, node: NodePattern, relationship: RelationshipPattern) -> Relation:
        ...
        relationship_relation: Relation = self._from_relationship_pattern(relationship=relationship)
        ...
        relationship_variable_column = relationship_relation.variable_map[relationship_variable]

        joined_relation: Relation = Join(
            left=node_relation,
            right=relationship_relation,
            source_algebraizable=PatternIntersection(pattern_list=[node, relationship]),
            on_left=[node_variable_column],
            on_right=[RELATIONSHIP_SOURCE_COLUMN],
            join_type=JoinType.INNER,
            variable_map={**node_relation.variable_map, **relationship_relation.variable_map},
            variable_type_map={**node_relation.variable_type_map, **relationship_relation.variable_type_map},
            column_names=node_relation.column_names + relationship_relation.column_names,
        )
        
        return joined_relation
    
    
    def _from_node_relationship_head(self, node: NodePattern, relationship: RelationshipPattern) -> Relation:
        ...
        joined_relation: Relation = Join(
            ...
            source_algebraizable=PatternIntersection(pattern_list=[node, relationship]),
            on_left=[node_variable_column],
            on_right=[RELATIONSHIP_TARGET_COLUMN],
            join_type=JoinType.INNER,
            variable_map={**node_relation.variable_map, **relationship_relation.variable_map},
            variable_type_map={**node_relation.variable_type_map, **relationship_relation.variable_type_map},
            column_names=node_relation.column_names + relationship_relation.column_names,
    ...
    def _from_intersection_list(self, relations: list[Relation]) -> Relation:
        ...
        for next_relation in relations[1:]:
            ...
            common_variables: set[Variable] = set(accumulated_value.variable_map.keys()).intersection(set(next_relation.variable_map.keys()))
            # Assume for now that there are common variables. We will need to perform a cross product in the future
            if not common_variables:
                raise NotImplementedError("No common variables found for join. Cross product not implemented yet.")
            # Perform the binary join using the common variables
            variable_map = {**accumulated_value.variable_map, **next_relation.variable_map}
            variable_type_map = {**accumulated_value.variable_type_map, **next_relation.variable_type_map}
            column_names = accumulated_value.column_names + next_relation.column_names
            ...
            accumulated_value = Join(
                ...
                right=next_relation,
                join_type=JoinType.INNER,
                on_left=[accumulated_value.variable_map[var] for var in common_variables],
                on_right=[next_relation.variable_map[var] for var in common_variables],
                variable_map=variable_map,
                variable_type_map=variable_type_map,
                column_names=column_names,
                source_algebraizable=PatternIntersection(pattern_list=[accumulated_value.source_algebraizable, next_relation.source_algebraizable])
            )
    ...
    def _binary_join(self, left: Relation, right: Relation) -> Relation:
        ...
        match (left.source_algebraizable, right.source_algebraizable):
            case (NodePattern(), RelationshipPattern()) if (
                cast(typ=RelationshipPattern, val=right.source_algebraizable).direction
                ...
                right_join_key: ColumnName = right.variable_map[
                    cast(typ=RelationshipPattern, val=right.source_algebraizable).variable
                ]
                join_type: JoinType = JoinType.INNER
                variable_map: VariableMap = {**left.variable_map, **right.variable_map}
                variable_type_map: VariableTypeMap = {**left.variable_type_map, **right.variable_type_map}
                join: Join = Join(
                    left=left,
                    right=right,
                ...
                variable_relation: Relation = SelectColumns(
                    ...
                    column_names=[
                        column_name for column_name in join.variable_map.values()
                    ],
                )
            ...
            case (NodePattern(), RelationshipPattern()) if (
                ...
                new_right.source_algebraizable = cast(RelationshipPattern, right.source_algebraizable).model_copy(update={"direction": RelationshipDirection.RIGHT})
                return self._binary_join(left=new_right, right=left)
            case (RelationshipPattern(), NodePattern()) if (
                cast(typ=RelationshipPattern, val=left.source_algebraizable).direction
                == RelationshipDirection.RIGHT
            ):
                LOGGER.debug(
                    msg="Joining Node with Relationship for -[r:RELATIONSHIP]->(n) (head)`."
            ...
            case (RelationshipPattern(), NodePattern()) if (
                ...
                new_left.source_algebraizable = cast(RelationshipPattern, left.source_algebraizable).model_copy(update={"direction": RelationshipDirection.RIGHT})
    ...
    def _from_relationship_pattern(
        ...
        relationship_relation: RelationshipTable = self.context.relationship_mapping[relationship.labels[0]]
        # new_column_name: ColumnName = random_hash()
        # new_source_column_name: ColumnName = random_hash()
        # new_target_column_name: ColumnName = random_hash()
        relation: Projection = Projection(
            relation=relationship_relation,
            projected_column_names = {ID_COLUMN: ID_COLUMN, RELATIONSHIP_SOURCE_COLUMN: RELATIONSHIP_SOURCE_COLUMN, RELATIONSHIP_TARGET_COLUMN: RELATIONSHIP_TARGET_COLUMN},
            variable_map={relationship.variable: ID_COLUMN},
    ...
    def _from_relationship_pattern_with_attrs(
        ...
        raise NotImplementedError("Filtering on relationship properties is not implemented yet.")
    ...
    def _from_node_pattern_no_attrs(self, node: NodePattern) -> Projection:
        ...
        entity_relation: EntityTable = self.context.entity_mapping[node.labels[0]]
        new_column_name: ColumnName = random_hash()
        relation: Projection = Projection(
            relation=entity_relation,
            projected_column_names = {ID_COLUMN: new_column_name},
    ...
    def _from_node_pattern_with_attrs(self, node: NodePattern) -> FilterRows:
        ...
        base_node_relation: Relation = self.to_relation(
            obj=base_node
        )
        ...
        filtered_relation: FilterRows = FilterRows(
            ...
            source_algebraizable=node,
            column_names=base_node_relation.column_names,
        )
