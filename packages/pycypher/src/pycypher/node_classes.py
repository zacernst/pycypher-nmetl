...
class Evaluable(TreeMixin, abc.ABC):  # pylint: disable=too-few-public-methods
    ...
    def _evaluate(
        ...
        raise NotImplementedError(
            f"Called _evaluate with {self.__class__.__name__}"
        )
...
class Where(Evaluable, TreeMixin):
    ...
    def _evaluate(
        ...
        if not self.predicate:
            return projection_list
        filtered_projection_list: ProjectionList = ProjectionList(
            projection_list=[]
        )
...
class Or(BinaryBoolean, Evaluable):
    ...
    def _evaluate(
        ...
        return Literal(
            left_value.value
            or right_value.value  # , parent_projection=projection
        )
