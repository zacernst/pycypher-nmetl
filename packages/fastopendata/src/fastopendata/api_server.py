...
class _TokenBucket:
    ...
    *max_tokens / window_seconds* per second.  A request consumes one
    ...
    def __init__(self, max_tokens: int, window_seconds: float) -> None:
        ...
        self.refill_rate = (
            max_tokens / window_seconds if window_seconds > 0 else max_tokens
        )
...
if _SITE_DIR.exists():
    app.mount(
        "/site", StaticFiles(directory=_SITE_DIR, html=True), name="site"
    )

##### Get the static redocs/docs stuff working
app.mount(
    "/static",
    StaticFiles(directory="./packages/fastopendata/src/fastopendata/static"),
    name="static",
)
