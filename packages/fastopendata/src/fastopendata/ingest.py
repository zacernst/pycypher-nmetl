...
def start_worker(worker_num, num_workers):
    ...
    PUMS_DATA_DICTIONARY_PATH = (
        f"{SOURCE_DIR}/acs_pums_2023_data_dictionary.json"
    )
    session = load_session_config(
        INGEST_CONFIG_PATH, worker_num=worker_num, num_workers=num_workers
    )
